...
resource "vault_namespace" "gke-demo" {
  path = var.vault_gke_namespace
}
...
resource "vault_jwt_auth_backend_role" "tfc_role" {
  ...
  bound_claims = {
    sub = "organization:${var.tfc_organization_name}:project:${var.tfc_project_name}:workspace:${var.tfc_workspace_name_vault}:run_phase:*"
  }
...
path "sys/policies/acl/vaulidate" {
